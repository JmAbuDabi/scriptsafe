...
const statuschange = async function() {
	...
	const forever = await bkg.getLocale("forever");
    $(this).after(
		disable + ': <span class="box box3" data-duration="5">5m</span> ' +
		'<span class="box box3" data-duration="15">15m</span> ' +
		'<span class="box box3" data-duration="30">30m</span> ' +
		'<span class="box box3" data-duration="60">1h</span> ' +
        '<span class="box box2" data-duration="">' + forever + '</span>'
	);
        $("span[data-duration]").bind("click", statuschanger);
};
const statuschanger = async function() {
	port.postMessage({url: taburl, tid: tabid});
	await bkg.statuschanger($(this).attr('data-duration'))
	window.close();
};
const revokealltemp = async function() {
	port.postMessage({url: taburl, tid: tabid});
	await bkg.revokeTemp();
		window.close();
};
const bulkhandle = function() {
	port.postMessage({url: taburl, tid: tabid});
	bulk($(this));
};
...
const x_removehandle = async function () {
...
const x_savehandle = async function () {
	port.postMessage({ url: taburl, tid: tabid });
...
document.addEventListener('DOMContentLoaded', async function () {
	...
	$("#pop_close").mouseup(function(e) { if (e.which != 3) window.close(); }).attr('title', await bkg.getLocale("close"));
	$("#pop_refresh").mouseup(function(e) { if (e.which != 3) chrome.tabs.reload(); window.close(); });
});

async function init() {
	$("#version").html(version);
	$("#pop_options").html(await bkg.getLocale("options"));
	chrome.tabs.query({active: true, currentWindow: true}, async function(tab) {
		tab = tab[0];
		taburl = tab.url;
		tabdomain = await bkg.extractDomainFromURL(taburl);
		if (tabdomain.substr(0,4) == 'www.') tabdomain = tabdomain.substr(4);
		tabid = tab.id;
		if (tabdomain == 'chrome.google.com' || taburl.indexOf('chrome-extension://') == 0) {
			$("#currentdomain").html(await bkg.getLocale("notfiltered"));
			$(".thirds").html('<i>'+ await bkg.getLocale("noexternal")+'</i>');
		} else {
			chrome.runtime.sendMessage({reqtype: "get-list", url: taburl, tid: tabid}, async function(response) {
				if (typeof response === 'undefined' || response == 'reload') {
					if (tab.url.substring(0, 4) == 'http') {
						$("table").html('<tr><td>'+await bkg.getLocale("recentlyupdated")+'</td></tr>');
					} else {
						$("table").html('<tr><td>'+ await bkg.getLocale("cannotprocess")+'</td></tr>');
					}
					return;
				}
				...
				var tabInTemp = await bkg.in_array(tabdomain, response.temp);
				var tabdomainfriendly = tabdomain.replace(/[.\[\]:]/g,"_");
				var tabdomainroot = await bkg.getDomain(tabdomain);
				if (response.rating == 'true') $("#currentdomain").html('<span class="domainname domainoutput"><span class="wot"><a href="http://www.mywot.com/en/scorecard/'+tabdomain+'" target="_blank" title="'+await bkg.getLocale("ratingbtn")+': '+tabdomain+'"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span>'+tabdomain+'</span>');
				...
				if ((responseBlockedCount == 0 && responseAllowedCount == 0) || response.status == 'false' || (response.mode == 'block' && (response.enable == '1' || response.enable == '4'))) {
					if (response.status == 'false') {
						...
						$(".thirds").css('text-align', 'center').html('<i>'+await bkg.getLocale("ssdisabled")+'</i>');
						$("#parent").css('text-align', 'center').append('<div class="box box1 snstatus" title="'+await bkg.getLocale("enabless")+'">'+await bkg.getLocale("enabless")+'</div>');
					...
					if (responseBlockedCount != 0) {
						if (response.domainsort == 'true') response.blockeditems = await bkg.domainSort(response.blockeditems);
						else response.blockeditems.sort();
						$(".thirds").parent().after("<tr><td class='bolded' style='height: 14px; padding-top: 5px;'><span class='blocked'>"+await bkg.getLocale("blockeditems")+"</span></td></tr><tr><td class='thirds' id='blocked'></td></tr>");
							...
							if (itemdomain) {
								...
								if ($('#blocked .thirditem[rel="x_'+itemdomainfriendly+'"]').length == 0) {
									if (domainCheckStatus == '1') {
										...
										var trustval1 = '';
										var allowedtype;
										var trustType = await bkg.trustCheck(itemdomain);
										if (trustType == '1') {
											trustval0 = ' selected';
											allowedtype = 3;
										} else if (trustType == '2') {
										...
										$("#blocked").append('<div class="thirditem" title="['+response.blockeditems[i][1]+'] '+$.trim(response.blockeditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="r_'+itemdomainfriendly+'"></span><span class="domainoutput">'+outputdomain+'</span> (<span rel="count_'+itemdomainfriendly+'">1</span>)</span><br /><span class="choices" rel="'+itemdomain+'" sn_list="'+allowedtype+'"><span class="box box4 x_'+itemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box1 x_trust'+trustval0+'" rel="3" title="Trust Entire Domain">'+await bkg.getLocale("trust")+'</span><span class="box box2 x_blacklist selected" rel="1" title="Deny">'+await bkg.getLocale("deny")+'</span><span class="box box2 x_trust'+trustval1+'" rel="4" title="Distrust Entire Domain">'+await bkg.getLocale("distrust")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
									} else {
										if (response.blockeditems[i][1] == 'NOSCRIPT' || response.blockeditems[i][1] == 'WEBBUG') {
											$("#blocked").append('<div class="thirditem" title="['+response.blockeditems[i][1]+'] '+$.trim(response.blockeditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span>&lt;'+response.blockeditems[i][1]+'&gt;</span> (<span rel="count_'+itemdomainfriendly+'">1</span>)</span></div>');
											...
											if ($('#blocked .fpcat[rel="x_'+itemdomainfriendly+'"]').length == 0) {
												$("#blocked").append('<div class="fpcat" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><div class="fphead">'+response.blockeditems[i][1]+' (<span rel="count_'+itemdomainfriendly+'">1</span>)<span class="chevron"></span></div><div class="fpoptions details_'+itemdomainfriendly+'"><div class="fpitem" title="['+response.blockeditems[i][1]+'] '+$.trim(response.blockeditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="fp_'+fpitemdomainfriendly+'" data-fpdomain="'+await bkg.getDomain(fpitemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="rfp_'+fpitemdomainfriendly+'"></span><span class="domainoutput">'+fpitemdomain+'</span> (<span rel="fpcount_'+fpitemdomainfriendly+'">1</span>)</span><br /><span class="choices fpchoices" rel="'+fpitemdomain+'" sn_list="'+itemdomain+'"><span style="display: none;" class="box box4 fp_'+fpitemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div></div></div>');
											} else {
												if ($('#blocked .fpcat[rel="x_'+itemdomainfriendly+'"] .fpitem[rel="fp_'+fpitemdomainfriendly+'"]').length == 0) $("#blocked .fpcat[rel='x_"+itemdomainfriendly+"'] .fpoptions").append('<div class="fpitem" title="['+response.blockeditems[i][1]+'] '+$.trim(response.blockeditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="fp_'+fpitemdomainfriendly+'" data-fpdomain="'+await bkg.getDomain(fpitemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="rfp_'+fpitemdomainfriendly+'"></span><span class="domainoutput">'+fpitemdomain+'</span> (<span rel="fpcount_'+fpitemdomainfriendly+'">1</span>)</span><br /><span class="choices fpchoices" rel="'+fpitemdomain+'" sn_list="'+itemdomain+'"><span style="display: none;" class="box box4 fp_'+fpitemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
											...
											$("#blocked").append('<div class="thirditem" title="['+response.blockeditems[i][1]+'] '+$.trim(response.blockeditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="r_'+itemdomainfriendly+'"></span><span class="domainoutput">'+itemdomain+'</span> (<span rel="count_'+itemdomainfriendly+'">1</span>)</span><br /><span class="choices" rel="'+itemdomain+'" sn_list="-1"><span class="box box4 x_'+itemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box1 x_trust" rel="3" title="Trust Entire Domain">'+await bkg.getLocale("trust")+'</span><span class="box box2 x_blacklist" rel="1" title="Deny">'+await bkg.getLocale("deny")+'</span><span class="box box2 x_trust" rel="4" title="Distrust Entire Domain">'+await bkg.getLocale("distrust")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
								...
								if (response.rating == 'true') {
									$("#blocked [rel='r_"+itemdomainfriendly+"']").html('<span class="wot"><a href="http://www.mywot.com/en/scorecard/'+itemdomain.replace(/[\[\]]/g,"")+'" target="_blank" title="'+await bkg.getLocale("ratingbtn")+': '+itemdomain+'"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span>');
									if (response.blockeditems[i][6]) $('#blocked [rel="rfp_'+fpitemdomainfriendly+'"]').html('<span class="wot"><a href="http://www.mywot.com/en/scorecard/'+fpitemdomain.replace(/[\[\]]/g,"")+'" target="_blank" title="'+await bkg.getLocale("ratingbtn")+': '+fpitemdomain+'"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span>');
								}
								if ((response.annoyances == 'true' && response.annoyancesmode == 'strict' && domainCheckStatus == '-1' && baddiesstatus == '1') || (response.antisocial == 'true' && baddiesstatus == '2')) {
									...
									if (response.antisocial == 'true' && baddiesstatus == '2') {
										$("#blocked [rel='x_"+itemdomainfriendly+"'] .x_blacklist").attr("title","Antisocial").html(await bkg.getLocale("antisocialpopup")).addClass("selected");
									} else {
										$("#blocked [rel='x_"+itemdomainfriendly+"'] .x_blacklist").attr("title","Unwanted Content Provider").html(await bkg.getLocale("unwanted")).addClass("selected");
									}
								} else if ((parentstatus == '1' || parentstatus == '-1') && domainCheckStatus == '0') {
									$("#blocked [rel='x_"+itemdomainfriendly+"'] .box1, #blocked [rel='x_"+itemdomainfriendly+"'] .x_trust, #blocked [rel='x_"+itemdomainfriendly+"'] .box3, #blocked [rel='x_"+itemdomainfriendly+"'] .box4").hide();
									$("#blocked [rel='x_"+itemdomainfriendly+"'] .x_blacklist").attr("title","Ignored allowed domain due to unlisted tab domain").html(await bkg.getLocale("ignoredallow")).addClass("selected");
								...
								if (mode == 'allow') {
									if (await bkg.in_array(itemdomain, response.temp)) {
					...
					if (responseAllowedCount != 0) {
						if (response.domainsort == 'true') response.alloweditems = await bkg.domainSort(response.alloweditems);
						else response.alloweditems.sort();
						$(".thirds").parent().parent().append("<tr><td class='bolded' style='height: 14px;'><span class='allowed'>"+await bkg.getLocale("alloweditems")+"</span></td></tr><tr><td class='thirds' id='allowed'></td></tr>");
							...
							if (itemdomain) {
								...
								if ($('#allowed .thirditem[rel="x_'+itemdomainfriendly+'"]').length == 0) {
									if (response.alloweditems[i][3] == '0') {
										...
										$("#allowed").append('<div class="thirditem" title="['+response.alloweditems[i][1]+'] '+$.trim(response.alloweditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="r_'+itemdomainfriendly+'"></span><span class="domainoutput">'+itemdomain+'</span> (<span rel="count_'+itemdomainfriendly+'">1</span>)</span><br /><span class="choices" rel="'+itemdomain+'" sn_list="'+allowedtype+'"><span class="box box4 x_'+itemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist selected" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box1 x_trust'+trustval0+'" rel="3" title="Trust Entire Domain">'+await bkg.getLocale("trust")+'</span><span class="box box2 x_blacklist" rel="1" title="Deny">'+await bkg.getLocale("deny")+'</span><span class="box box2 x_trust'+trustval1+'" rel="4" title="Distrust Entire Domain">'+await bkg.getLocale("distrust")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
										...
										if (response.alloweditems[i][5]) {
											if ($('#allowed .fpcat[rel="x_'+itemdomainfriendly+'"]').length == 0) {
												$("#allowed").append('<div class="fpcat" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><div class="fphead">'+response.alloweditems[i][1]+'<span class="chevron"></span></div><div class="fpoptions details_'+itemdomainfriendly+'"><div class="fpitem" title="['+response.alloweditems[i][1]+'] '+$.trim(response.alloweditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="fp_'+fpitemdomainfriendly+'" data-fpdomain="'+await bkg.getDomain(fpitemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="rfp_'+fpitemdomainfriendly+'"></span><span class="domainoutput">'+fpitemdomain+'</span></span><br /><span class="choices fpchoices" rel="'+fpitemdomain+'" sn_list="'+itemdomain+'"><span style="display: none;" class="box box4 fp_'+fpitemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div></div></div>');
											} else {
												if ($('#allowed .fpcat[rel="x_'+itemdomainfriendly+'"] .fpitem[rel="fp_'+fpitemdomainfriendly+'"]').length == 0) $("#allowed .fpcat[rel='x_"+itemdomainfriendly+"'] .fpoptions").append('<div class="fpitem" title="['+response.alloweditems[i][1]+'] '+$.trim(response.alloweditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="fp_'+fpitemdomainfriendly+'" data-fpdomain="'+await bkg.getDomain(fpitemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="rfp_'+fpitemdomainfriendly+'"></span><span class="domainoutput">'+fpitemdomain+'</span></span><br /><span class="choices fpchoices" rel="'+fpitemdomain+'" sn_list="'+itemdomain+'"><span style="display: none;" class="box box4 fp_'+fpitemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
											}
										} else {
											$("#allowed").append('<div class="thirditem" title="['+response.alloweditems[i][1]+'] '+$.trim(response.alloweditems[i][0].replace(/"/g, "'").replace(/\&lt;/g, "<").replace(/\&gt;/g, ">").replace(/\&amp;/g, "&"))+'" rel="x_'+itemdomainfriendly+'" data-domain="'+await bkg.getDomain(itemdomain)+'" data-baddie="'+baddiesstatus+'"><span><span rel="r_'+itemdomainfriendly+'"></span><span class="domainoutput">'+itemdomain+'</span> (<span rel="count_'+itemdomainfriendly+'">1</span>)</span><br /><span class="choices" rel="'+itemdomain+'" sn_list="-1"><span class="box box4 x_'+itemdomainfriendly+'" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</span><span class="box box1 x_whitelist" rel="0" title="Allow Domain">'+await bkg.getLocale("allow")+'</span><span class="box box1 x_trust" rel="3" title="Trust Entire Domain">'+await bkg.getLocale("trust")+'</span><span class="box box2 x_blacklist" rel="1" title="Deny">'+await bkg.getLocale("deny")+'</span><span class="box box2 x_trust" rel="4" title="Distrust Entire Domain">'+await bkg.getLocale("distrust")+'</span><span class="box box3 x_bypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</span></span></div>');
								...
								if (response.rating == 'true') {
									$("#allowed [rel='r_"+itemdomainfriendly+"']").html('<span class="wot"><a href="http://www.mywot.com/en/scorecard/'+itemdomain.replace(/[\[\]]/g,"")+'" target="_blank" title="'+await bkg.getLocale("ratingbtn")+': '+itemdomain+'"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span>');
									if (response.alloweditems[i][5]) $('#allowed [rel="rfp_'+fpitemdomainfriendly+'"]').html('<span class="wot"><a href="http://www.mywot.com/en/scorecard/'+fpitemdomain.replace(/[\[\]]/g,"")+'" target="_blank" title="'+await bkg.getLocale("ratingbtn")+': '+fpitemdomain+'"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span>');
								}
								if (response.annoyances == 'true' && baddiesstatus == '1') {
									$("#allowed [rel='x_"+itemdomainfriendly+"'] .x_blacklist").attr("title","Unwanted Content Provider").html(await bkg.getLocale("unwanted"));
								} else if (itemdomain[0] == '[' || itemdomain.match(/^((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){3}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})$/g)) {
					...
					if (mode == 'block') {
						if ($('#blocked .thirditem').length == 1 && ($('#blocked .thirditem[rel="x_no_script"]').length == 1 || $('#blocked .thirditem[rel="x_web_bug"]').length == 1 || $('#blocked .thirditem[rel="x_canvas_fingerprint"]').length == 1 || $('#blocked .thirditem[rel="x_canvas_font_access"]').length == 1 || $('#blocked .thirditem[rel="x_audio_fingerprint"]').length == 1 || $('#blocked .thirditem[rel="x_webgl_fingerprint"]').length == 1 || $('#blocked .thirditem[rel="x_battery_fingerprint"]').length == 1 || $('#blocked .thirditem[rel="x_device_enumeration"]').length == 1 || $('#blocked .thirditem[rel="x_gamepad_enumeration"]').length == 1 || $('#blocked .thirditem[rel="x_webvr_enumeration"]').length == 1 || $('#blocked .thirditem[rel="x_bluetooth_enumeration"]').length == 1) || $('#blocked .thirditem[rel="x_spoofed_timezone"]').length == 1 || $('#blocked .thirditem[rel="x_client_rectangles"]').length == 1 || $('#blocked .thirditem[rel="x_clipboard_interference"]').length == 1 || $('#blocked .thirditem[rel="x_data_url"]').length == 1 || $('#blocked .thirditem[rel="x_browser_plugins_enumeration"]').length == 1) {
							...
							if ($("#blocked .x_whitelist:visible").length != 0) {
								$(tempSel).append('<br /><div class="box box3 allowsession" title="Allow all blocked items for the session (not including webbugs/noscript/fingerprinting/annoyances)">'+await bkg.getLocale("allowallblocked")+'</div>');
							} else {
								$(tempSel).append('<br />');
							}
						}
					} else {
						$(tempSel).append('<br /><div class="box box3 allowsession" title="Block all allowed items for the session">'+await bkg.getLocale("blockallallowed")+'</div>');
					}
					$(".allowsession").bind("click", bulkhandle);
					if (intemp || tabInTemp) {
						if ($(tempSel+' > br').length == 0) $(tempSel).append('<br />');
						$(tempSel).append('<div class="box box5 prevoke" title="Revoke temporary permissions given to the current page">'+await bkg.getLocale("revoketemp")+'</div>');
				...
				if (typeof response.temp !== 'undefined' && response.temp.length || (typeof response.tempfp !== 'undefined' && response.tempfp)) {
					if ($(tempSel+' > br').length == 0) $(tempSel).append('<br />');
					$(tempSel).append('<div class="box box5 clearglobaltemp" title="Revoke all temporary permissions given in this entire browsing session">'+await bkg.getLocale("revoketempall")+'</div>');
					$(".clearglobaltemp").bind("click", revokealltemp);
				} else if ($(tempSel+' > br').length == 1 && $(tempSel+' > div.allowsession').length == 0) $(tempSel+' > br').remove();
				$("#parent").prepend('<div class="box box4 pclear" title="Clear Domain from List">'+await bkg.getLocale("clear")+'</div><div class="box box1 pallow" rel="0" title="Allow Current Domain">'+await bkg.getLocale("allow")+'</div><div class="box box1 ptrust" rel="3" title="Trust Entire Domain">'+await bkg.getLocale("trust")+'</div><div class="box box2 pdeny" rel="1" title="Deny">'+await bkg.getLocale("deny")+'</div><div class="box box2 ptrust" rel="4" title="Distrust Entire Domain">'+await bkg.getLocale("distrust")+'</div><div class="box box3 pbypass" rel="2" title="Temporary">'+await bkg.getLocale("temp")+'</div>').attr("sn_list",response.enable);
				...
				if (response.enable == '1' || response.enable == '4') {
					...
					var domainCheckStatus = await bkg.domainCheck(taburl, 1);
					var baddiesStatus = await bkg.baddies(taburl, response.annoyancesmode, response.antisocial);
					if ((response.annoyances == 'true' && response.annoyancesmode == 'strict' && domainCheckStatus == '-1' && baddiesStatus == 1) || (response.antisocial == 'true' && baddiesStatus == '2')) {
						if (response.antisocial == 'true' && baddiesStatus == '2') {
							$(".pdeny").addClass("selected").attr("title","Blocked (antisocial)").text(await bkg.getLocale("antisocialpopup"));
						} else {
							$(".pdeny").addClass("selected").attr("title","Blocked (provider of unwanted content)").text(await bkg.getLocale("blocked"));
						}
				...
				if (response.status == 'true') $("#credit").append('&nbsp;|&nbsp;<span class="box box2 snstatus" title="Disable ScriptSafe">'+await bkg.getLocale("disable")+'</span>');
...
async function save(url, el, type) {
	...
	if (el.parent().hasClass("fpchoices")) {
		...
		if (val < 2) {
			...
			await bkg.topHandler(url, fpList);
			val = 0;
		}
	} else {
		if (val < 2) {
