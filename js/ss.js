...
function ScriptSafe() {
	...
	if (SETTINGS['APPLET'] == 'true') $("applet[data-ss" + timestamp + "!='1']").each(function () { let elSrc = $(this).attr('code'); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'APPLET' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'APPLET' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['VIDEO'] == 'true') $("video[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'VIDEO' }); removeMedia($(this)); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'VIDEO' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['AUDIO'] == 'true') $("audio[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'AUDIO' }); removeMedia($(this)); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'AUDIO' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['IFRAME'] == 'true') $("iframe[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'FRAME' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'FRAME' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['OBJECT'] == 'true') $("object[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'OBJECT' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'OBJECT' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['EMBED'] == 'true') $("embed[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'EMBED' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'EMBED' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['IMAGE'] == 'true') $("picture[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'IMAGE' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'IMAGE' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	if (SETTINGS['IMAGE'] == 'true') $("img[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'IMAGE' }); $(this).remove(); } else { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: 'IMAGE' }); $(this).attr("data-ss" + timestamp, '1'); } } });
	/* Fallback Inline Script Handling */
	if (SETTINGS['SCRIPT'] == 'true' && SETTINGS['EXPERIMENTAL'] == '0') {
		clearUnloads();
		$("script[data-ss" + timestamp + "!='1']").each(function () { let elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.runtime.sendMessage({ reqtype: "update-blocked", src: elSrc, node: 'SCRIPT' }); $(this).remove(); } else { if (elSrc.substr(0, 4) == 'http') { chrome.runtime.sendMessage({ reqtype: "update-allowed", src: elSrc, node: "SCRIPT" }); $(this).attr("data-ss" + timestamp, '1'); } } } });
...
function relativeToAbsoluteUrl(url) { // credit: NotScripts
	...
	let base = document.baseURI.match(/.+\//);
	if (!base)
